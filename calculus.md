...
- Сохраняют круговую гладкость;
- Отображают окружности в окружности;
- Сохраняют меру угла;
- Инвариантны относительно интегралов вида **\(\int f(z)\,dz/iz\)**.

Эти преобразования формируют структуру, допускающую композиции с двойственной и самоподобной симметрией. Их можно трактовать как фундаментальные изометрии в круговом пространстве, а также как функциональный гейт-сет, из которого строятся более сложные преобразования.
...
3. **Инверсия**  
   ...
   — отражение относительно единичного круга

4. **Смещение**  
   <div style="background:#f9f9f9; padding: 0.5em; border-left: 3px solid #ccc;">  
   $$ T_c(z) = z + c $$  
   </div>  
   — сдвиг центра вращения

**Смысл:**

Эти четыре операции формируют универсальный набор, из которого можно получить широкий спектр круговых и спиральных преобразований. Их композиции реализуют обобщённые действия, инвариантные при вращении и масштабировании, и служат базой для построения операторных алгебр в круговом анализе.
